...
def car_wear(car: dict) -> float:
    """Return wear percentage for a car, or 0.0 if no last-service reading is present."""
...
def fleet_summary(fleet: list[dict]) -> dict:
    """Return a summary dict with count, due count, and average wear across the fleet."""
    total = 0.0
    due = 0
    for car in fleet:
        total += car_wear(car)
